...
class de_Boor
{
   ...
   class point
   {
      public:
         ...
         friend point operator*( const double lhs,
                                 const point & rhs
                               )
         {
         ...
         for( int i = 0, c0 = 0, c1 = 0; i < 100; i++ )
         {
            do
            {
               ...
               // Check to make sure the color is not too dark.
                      ...
                      ( c0 & 0xff00 ) < 0xa000 &&
                      ( c0 & 0xff0000 ) < 0xa00000
                    ) ||
                    ( ( c1 & 0xff ) < 0xa0 &&
                      ( c1 & 0xff00 ) < 0xa000 &&
                      ( c1 & 0xff0000 ) < 0xa00000
                    )
                  );
                     ...
                     )
      {
         ...
         { 
            ...
            case DECREASE_ITERATION_CONSTANT:
               ...
               break;
            ////////////////////////////////////////////////////////
            case TOGGLE_CONTROL_POINT_TEXT:
...
/*
            if( subdWt.empty() )
            {
               // subdWt.push_back( pairLS( p1, point( 0.5 - minMaxT, 0.5 ) ) );
               // subdWt.push_back( pairLS( p1, point( 0.5 + minMaxT, 0.5 ) ) );
            ...
            if( shellBl == true )
            {
               ...
               for( int d = 0; d < numControlPts - 1; d++ )
               {
                  ...
                  for( int u = d + 1; u < u1; u++ )
                  {
                     ...
                     {
                        drawLine( d0 + ( d1 - d0 ) * ( ( knots[ u ] - knots[ d + 1 ] ) /
                                                       ( knots[ u1 ] - knots[ d + 1 ] )
                                                     ),

                                  d0 + ( d1 - d0 ) * ( ( knots[ u + 1 ] - knots[ d + 1 ] ) /
                                                       ( knots[ u1 ] - knots[ d + 1 ] )
                                                     ),
                                  u
                                );
   ...
   private:
      ...
      {  
         ...
         for( j = 1; j <= degree - r; j++ )
         {
            ...
            {
               tempPt[ k ] = ( ( knots[ degree + i - j + 1 ] - dt ) /
                               ( knots[ degree + i - j + 1 ] - knots[ i ] )
                             ) * tempPt[ k ] +
         ...
         {  
            ...
            if( center.isProjectable() == true )
            {
               ...
               circleDrawingPrimitive( fti( center.x ),
                                       fti( center.y ),
                                       radius,
                                       colors0[ (int) knots[ ( knotIndex ) % 100 ] % 100 ],
                                       colors0[ (int) knots[ ( knotIndex ) % 100 ] % 100 ]
                                       // colors0[ (int) knots[ ( knotIndex + 1 ) % 100  ] % 100 ]
                                     );
            }
         }
      }
      ...
      {	
         ...
         int size = knots.size() - degree;

         // Check if the knot sequence is long enough to handle all of these control points.
         // if( size > numControlPts )
         {
            // If we have enough knots ( or more than we need ) then we are ok.
         }
         // else
         {
            // We need to insert a knot at the end of the sequence,
            // since we need to have at least as many knots as
...
extern "C" extern int init( double _halfWidth,
                            double _halfHeight,
                            void ( *_circleDrawingPrimitive )( int, int, int, int, int ),
                            void ( *_lineDrawingPrimitive )( int, int, int, int, int, int ),
                            void ( *_pointDrawingPrimitive )( int, int, int ),
                          ...
                          )
{
   ...
   if( !deBoorList )
   {
      ...
      ( *deBoorList )[ 0 ]->setPrimitiveDrawingFunctions( _halfWidth,
                                                          _halfHeight,
                                                          _circleDrawingPrimitive,
                                                          _lineDrawingPrimitive,
                                                          _pointDrawingPrimitive,
                          ...
                          )
{
   ...
   switch( inputEvent )
   {
      case de_Boor::ADD_DE_BOOR_CURVE:
      ...
      case de_Boor::REMOVE_DE_BOOR_CURVE:
      ...
      case de_Boor::TRAVERSE_DE_BOOR_CURVE_LIST:
