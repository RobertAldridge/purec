...
#define RND() ( ( (f32) rand() ) / ( (f32) ( RAND_MAX >> 1 ) ) -1 ) // calculates float between -1 and 1
...
// HEIGHT OF RASTER DEVICE LIMITED TO 4096
static u8  LFbitDepth,*LFbackBufferArray[4096];
static u16 LFwidth,LFheight;

static const s16 dotPosX[]       = {  -4,  -3,  -2,  -1,   0,   1,   2,   3};
static const s16 dotPosY[]       = {       -3,  -2,  -1,   0,   1,   2,   3};

static const u32 dotColorArray[] = {   0,   1,   8,  19,  23,  13,   2,   0,
									   1,  13,  56, 119, 136,  92,  29,   4,
									   5,  42, 143, 228, 234, 198,  92,  14,
									   8,  65, 192, 255, 255, 234, 135,  23,
									   7,  56, 174, 253, 254, 227, 118,  19,
								       3,  26,  95, 175, 191, 143,  55,   8,
								       1,  5,   25,  55,  66,  43,  13,   2
							       };

// OS DEPENDANT CODE
static LPDIRECTDRAWSURFACE LFOSbackBuffer;

extern void __LinearFrameBufferOSSetBackBuffer(void * /* LPDIRECTDRAWSURFACE */ BackBuffer,
											  ...
											  )
{
	...
	LFOSbackBuffer = (LPDIRECTDRAWSURFACE) BackBuffer;

	LFwidth        = BackBufferViewPortWidth;
	LFheight       = BackBufferViewPortHeight;

	assert( LFheight <= 4096 );

	LFOSbackBuffer->lpVtbl->GetSurfaceDesc( LFOSbackBuffer, &ddsd );
	...
	switch( ddsd.ddpfPixelFormat.dwRGBBitCount )
	{
		...
		case 15:
		case 16:
			if( ddsd.ddpfPixelFormat.dwGBitMask == 0x3e0 ) // if 5-5-5 15 bit mode
			{
			...
			break;
		case 24:
				assert( ddsd.ddpfPixelFormat.dwRBitMask == 0xff0000 &&
		...
		case 32:
			...
			break;
		default:
			assert( "__LinearFrameBufferOSSetBackBuffer - unsupported bit depth" && 0 );
			break;
	}

	LFOSbackBuffer->lpVtbl->Lock( LFOSbackBuffer, 0, &ddsd, DDLOCK_WAIT | DDLOCK_SURFACEMEMORYPTR,NULL );

	LFbackBufferArray[ 0 ] = (u8*) ddsd.lpSurface;

	loop = 1;

	do
	{
		LFbackBufferArray[ loop ] = LFbackBufferArray[ loop - 1 ] + ddsd.lPitch;

	}while( ++loop != LFheight );

	LFOSbackBuffer->lpVtbl->Unlock( LFOSbackBuffer, ddsd.lpSurface );
}

extern void __LinearFrameBufferLock(
								   )
{
	int           loop;
	DDSURFACEDESC ddsd;
	...
	if( LFbackBufferArray[ 0 ] != ddsd.lpSurface || LFbackBufferArray[ 1 ] != ( (u8*) ddsd.lpSurface ) + ddsd.lPitch )
	{
		LFbackBufferArray[ 0 ] = (u8*) ddsd.lpSurface;
...
extern __forceinline void __LinearFrameBufferUnLock(
												   )
{
	LFOSbackBuffer->lpVtbl->Unlock( LFOSbackBuffer, LFbackBufferArray[ 0 ] );
}
// END OS DEPENDANT CODE

extern __forceinline const u8    __LinearFrameBufferGetBitDepth()        { return LFbitDepth;        }
...
extern __forceinline u8 ** const __LinearFrameBufferGetBackBufferArray() { return LFbackBufferArray; }
		...
		if( particle->flags == PARTICLE_ACTIVE )
		{
			...
			else
			{
				...
				if( x >= 0 && x < width && y >= 0 && y < height && z >= .000001f && z <= 1 )
				{
					...
					do
					{
						// local anti-alias variables
						u32 DstRed,DstGreen,DstBlue;
						u32 SrcRed,SrcGreen,SrcBlue;
						u32 DstRatio;
						u32 SrcRatio;

						u32 ColorDst,ColorSrc;
						//

						dotWidthLoop = 0;

						dotY = (u16) ( y + dotPosY[ dotHeightLoop ] );

						do
						{
							dotX = (u16) ( x + dotPosX[ dotWidthLoop ] );
							...
							{
								dotColor = dotColorArray[ dotWidthLoop + ( dotHeightLoop << 3 ) ];
								...
								switch( bitDepth )
								{
									case 8:
										...
										ColorDst = (u8) ( ( red + green + blue ) / 3 );
										
										ColorSrc = *( bb[ dotY ] + dotX );

										DstRatio = 0xff - SrcRatio;

										*( bb[ dotY ] + dotX ) = (u8) ( ( ( ColorDst * SrcRatio ) + ( ColorSrc * DstRatio ) ) / 0xff );
									...
									case 15:
										...
										ColorSrc = *( ( (u16*) bb[ dotY ] ) + dotX );

										SrcRed   = ( ( ColorSrc << 17 ) >> 27 );
										SrcGreen = ( ( ColorSrc << 22 ) >> 27 );
										SrcBlue  = ( ( ColorSrc << 27 ) >> 27 );

										DstRatio = 0x1f - SrcRatio;

										*( ( (u16*) bb[ dotY ] ) + dotX ) = (u16) (
											( ( ( ( DstRed   * SrcRatio ) + ( SrcRed   * DstRatio ) ) / 0x1f ) << 10 ) |
											( ( ( ( DstGreen * SrcRatio ) + ( SrcGreen * DstRatio ) ) / 0x1f ) << 5  ) |
											  ( ( ( DstBlue  * SrcRatio ) + ( SrcBlue  * DstRatio ) ) / 0x1f )
											);
									...
									case 16:
										...
										SrcRed   = ( ( ColorSrc << 16 ) >> 27 );
										SrcGreen = ( ( ColorSrc << 21 ) >> 26 );
										SrcBlue  = ( ( ColorSrc << 27 ) >> 27 );

										DstRatio = 0x3f - SrcRatio;

										*( ( (u16*) bb[ dotY ] ) + dotX ) = (u16) (
											( ( ( ( DstRed   * SrcRatio ) + ( SrcRed   * DstRatio ) ) / 0x3f ) << 11 ) |
											( ( ( ( DstGreen * SrcRatio ) + ( SrcGreen * DstRatio ) ) / 0x3f ) << 5  ) |
											  ( ( ( DstBlue  * SrcRatio ) + ( SrcBlue  * DstRatio ) ) / 0x3f )
											);
									...
									case 24:
										...
										*( (u32*) ( bb[ dotY ] + dotX * 3 ) ) = ( ColorSrc & 0xff000000 ) | (
											( ( ( ( DstRed   * SrcRatio ) + ( SrcRed   * DstRatio ) ) / 0xff ) << 16 ) |
											( ( ( ( DstGreen * SrcRatio ) + ( SrcGreen * DstRatio ) ) / 0xff ) << 8  ) |
											  ( ( ( DstBlue  * SrcRatio ) + ( SrcBlue  * DstRatio ) ) / 0xff )
											);
						...
						}while( ++dotWidthLoop != 8 );

					}while( ++dotHeightLoop != 7 );
				}
			}
		}
