...
#define ALOGE(...) do{ __android_log_print(ANDROID_LOG_ERROR, OVR_LOG_TAG, __VA_ARGS__); }while(0)
#define ALOGW(...) do{ __android_log_print(ANDROID_LOG_WARN, OVR_LOG_TAG, __VA_ARGS__); }while(0)
#define ALOGV(...) do{ __android_log_print(ANDROID_LOG_VERBOSE, OVR_LOG_TAG, __VA_ARGS__); }while(0)
...
template <typename TChain, typename TAppend> void appendToNextChain(TChain& chainRoot, TAppend& toAppend)
{
...
)
{
  ...
  // Put it all together.
  const OVR::Matrix3f T_DestCoord_SourceCoord =
    T_DestCoord_DestNormCoord *
...
void android_main(struct android_app* androidApp)
{
  ...
  if(
    eglVersion < graphicsRequirements.minApiVersionSupported ||
  ...
  OXR(xrEnumerateViewConfigurations(app.Instance, app.SystemId, 0, &viewportConfigTypeCount, NULL) );
  ...
  XrEnvironmentDepthSwapchainStateMETA environmentDepthSwapchainState {XR_TYPE_ENVIRONMENT_DEPTH_SWAPCHAIN_STATE_META};

  OXR(xrGetEnvironmentDepthSwapchainStateMETA(app.EnvironmentDepthSwapchain, &environmentDepthSwapchainState) );
...
template <typename Result> Result JNI_CheckResult(Result result, const char* function, JNIEnv* env)
{
...
#define JNI_CHECK_RESULT(func) JNI_CheckResult(func, #func, env);
  ...
  jclass clsPackageManager = JNI_CHECK_RESULT(env->FindClass("android/content/pm/PackageManager") );

  jfieldID fidPermissionGranted = JNI_CHECK_RESULT(
    env->GetStaticFieldID(clsPackageManager, "PERMISSION_GRANTED", "I")
  );

  jint intPermissionGranted = JNI_CHECK_RESULT(env->GetStaticIntField(clsPackageManager, fidPermissionGranted) );
