        ...
        switch (baseEventHeader->type) {
            ...
            case XR_TYPE_EVENT_DATA_SPACES_SAVE_RESULT_META: {
                ...
                if (itr == SaveForSharingEventMap.end()) {
                    ALOGE(
                        "xrPollEvent: no requestId found in map for XR_TYPE_EVENT_DATA_SPACES_SAVE_RESULT_META");
                    break;
                }
                if (saveResult->result < XR_SUCCESS) {
...
static void SaveAnchorsAndTrySharing(ovrApp& app) {
    ...
    if (!app.IsLocalMultiplayerSupported) {
        ALOGW(
            "SaveAnchorsAndTrySharing: Local multiplayer is not supported, will only save not share.");
    } else {
...
int main() {
    ...
    // Check the list of required extensions against what is supported by the runtime.
    {
        ...
        for (uint32_t i = 0; i < numRequiredExtensions; i++) {
            if (!isExtensionEnumerated(
                    requiredExtensionNames[i], extensionProperties.data(), numOutputExtensions)) {
    ...
    OXR(xrGetInstanceProcAddr(
        instance, "xrSaveSpacesMETA", (PFN_xrVoidFunction*)(&app.FunPtrs.xrSaveSpacesMETA)));
    OXR(xrGetInstanceProcAddr(
        instance, "xrEraseSpacesMETA", (PFN_xrVoidFunction*)(&app.FunPtrs.xrEraseSpacesMETA)));
    OXR(xrGetInstanceProcAddr(
