...
static SsmmNode* SSMM_CHUNK_NODE_TO_NODE_REVERSE_SEQUENTIAL(ssmm* _this, SsmmNode* node);
static SsmmNode* SSMM_CHUNK_POOL_TO_NODE_INDEX(ssmm* _this, SsmmPool* pool, int index);
...
SsmmNode* SSMM_CHUNK_NODE_TO_NODE_REVERSE_SEQUENTIAL(ssmm* _this, SsmmNode* node) { return (SsmmNode*)( (char*)node - (ptrdiff_t)_this->sizeOf); }
SsmmNode* SSMM_CHUNK_POOL_TO_NODE_INDEX(ssmm* _this, SsmmPool* pool, int index) { return (SsmmNode*)( (char*)pool + (SsmmGetSizeOfPool() + (ptrdiff_t)index * (ptrdiff_t)_this->sizeOf) ); }
...
void* SsmmStackPush(ssmm* _this, void* memory/*chunk*/)
{
  void* result = 0;
...
void* SsmmStackPop(ssmm* _this)
{
  bool result = false;

  SsmmNode* chunk = 0;

  if( !_this|| _this->num <= 0)
    goto error;
...
//chunk = (SsmmNode*)memory[0];
  ...
  // this doesn't work if we're at the beginning of a pool
  //
  // in that case we'll have to walk backwards to the sequentially prior pool

  if(_this->freeChunks == SSMM_CHUNK_POOL_TO_FIRSTNODE(_this->poolTail) )
  {
    // get pointer to client data at the last element of prior pool
    chunk = SSMM_CHUNK_POOL_TO_NODE_INDEX(_this, _this->poolTail->previous, _this->poolTail->previous->num - 1);
...
#endif
    //_this->num no change
    //_this->most no change
    _this->max -= _this->poolTail->num;
    //_this->freeChunks = chunk;

    SsmmPool* poolTail = _this->poolTail;

    _this->poolTail = poolTail->previous;

    poolTail->previous = _this->freePools;
    _this->freePools = poolTail;
#if 0
SsmmPool* pool = _this->freePools;
_this->freePools = pool->previous;
_this->freeChunks = SSMM_CHUNK_POOL_TO_FIRSTNODE(pool);
_this->max += pool->num;
#endif
    // now time travel the entire instance back before the resize. the pool we are moving from will be added to the freePools list
  }
  else
  {
    chunk = SSMM_CHUNK_NODE_TO_NODE_REVERSE_SEQUENTIAL(_this, chunk);
  }
