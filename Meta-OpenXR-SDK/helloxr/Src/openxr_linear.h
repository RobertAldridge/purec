...
#define DEFAULT_NEAR_Z 0.015625f
...
inline static float XrVector2f_Length(const XrVector2f* v)
{
...
inline static float XrVector3f_Dot(const XrVector3f* a, const XrVector3f* b)
{
...
inline static void XrQuaternionf_CreateFromAxisAngle(
...
inline static void XrQuaternionf_Lerp(
  XrQuaternionf* result,
...
inline static void XrMatrix4x4f_CreateRotationRadians(
  XrMatrix4x4f* result,
...
inline static void XrMatrix4x4f_CreateRotation(
  XrMatrix4x4f* result,
...
)
{
  XrMatrix4x4f_CreateRotationRadians(
    result,
    degreesX * (MATH_PI / 180.0f),
    degreesY * (MATH_PI / 180.0f),
...
inline static void XrMatrix4x4f_CreateTranslationRotationScale(
  XrMatrix4x4f* result,
...
inline static void XrMatrix4x4f_CreateProjection(
  XrMatrix4x4f* result,
...
inline static void XrMatrix4x4f_CreateProjectionFov(
  XrMatrix4x4f* result,
  GraphicsAPI graphicsApi,
  const XrFovf fov,
  const float nearZ,
...
{
  return fabsf(matrix->m[3] ) <= epsilon &&
    fabsf(matrix->m[7] ) <= epsilon &&
...
inline static void XrMatrix4x4f_GetRotation(XrQuaternionf* result, const XrMatrix4x4f* src)
{
  ...
  const float m[9] = {
    src->m[0] * rcpScaleX, src->m[1] * rcpScaleX, src->m[2] * rcpScaleX,
...
inline static void XrMatrix4x4f_TransformBounds(
  XrVector3f* resultMins,
  XrVector3f* resultMaxs,
  const XrMatrix4x4f* matrix,
  const XrVector3f* mins,
  const XrVector3f* maxs
)
