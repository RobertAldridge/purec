...
static void compute_sorted_huffman(Codebook *c, uint8 *lengths, uint32 *values)
{
   ...
   // OPTIMIZATION: don't inc_lude the short ones, since they'll be caught by FAST_HUFFMAN.
...
void inverse_mdct_slow(float *buffer, int n)
{
   int i, j;
   int n2 = n >> 1;
   float* x = (float*)calloc(n2, sizeof(*x) );
...
int stb_vorbis_decode_filename(const char *filename, int *channels, int *sample_rate, short **output)
{
   ...
   *channels = v->channels;
   ...
   offset = data_len = 0;
   total = limit;
   data = (short*)calloc(total, sizeof(*data) );
   if(data == NULL) {
      stb_vorbis_close(v);
      return -2;
   }
...
/* Version history
    ...
    1.0     - 2014/05/26 - fix memory leaks; fix warnings; fix bugs in multichannel
                           (API change) report sample rate for decode-full-file funcs
    0.99996 - bracket #inc_lude <malloc.h> for macintosh compilation by Laurent Gomila
