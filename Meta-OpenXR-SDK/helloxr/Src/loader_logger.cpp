...
bool LoaderLogRecorder::LogDebugUtilsMessage(XrDebugUtilsMessageSeverityFlagsEXT /*message_severity*/, XrDebugUtilsMessageTypeFlagsEXT /*message_type*/, const XrDebugUtilsMessengerCallbackDataEXT* /*callback_data*/)
{
  return false;
}
...
{/*
  ...
  if(!debug_string.empty() )
  {
    ...
    if(debug_string == "error")
      ...
      debug_flags = XR_LOADER_LOG_MESSAGE_SEVERITY_ERROR_BIT | XR_LOADER_LOG_MESSAGE_SEVERITY_WARNING_BIT | XR_LOADER_LOG_MESSAGE_SEVERITY_INFO_BIT;
...
{/*
  std::unique_lock<std::shared_timed_mutex> lock(_mutex);
  vector_remove_if_and_erase(_recorders, [=](std::unique_ptr<LoaderLogRecorder> const& recorder) { return recorder->UniqueId() == unique_id; } );
...
bool LoaderLogger::LogMessage(XrLoaderLogMessageSeverityFlagBits message_severity, XrLoaderLogMessageTypeFlags message_type, const std::string& message_id, const std::string& command_name, const std::string& message, const std::vector<XrSdkLogObjectInfo>& objects)
{/*
  ...
  {
    if( (recorder->MessageSeverities() & message_severity) == message_severity && (recorder->MessageTypes() & message_type) == message_type)
...
bool LoaderLogger::LogDebugUtilsMessage(XrDebugUtilsMessageSeverityFlagsEXT message_severity, XrDebugUtilsMessageTypeFlagsEXT message_type, const XrDebugUtilsMessengerCallbackDataEXT* callback_data)
{
  bool exit_app = false;/*
  ...
  for(std::unique_ptr<LoaderLogRecorder>& recorder : _recorders)
  {
    ...
    if(recorder->Type() != XR_LOADER_LOG_DEBUG_UTILS || (recorder->MessageSeverities() & log_message_severity) != log_message_severity || (recorder->MessageTypes() & log_message_type) != log_message_type)
      continue;
