...
struct MemoryAllocator
{
  void Init(VkPhysicalDevice physicalDevice, VkDevice device)
  {
    ...
    if(tableVk.GetPhysicalDeviceMemoryProperties)
      tableVk.GetPhysicalDeviceMemoryProperties(physicalDevice, &m_memProps);
  }
      ...
      if( (memReqs.memoryTypeBits & (1 << i) ) != 0u)
      {
        ...
        if( (m_memProps.memoryTypes[i].propertyFlags & flags) == flags)
        {
          ...
          if(tableVk.AllocateMemory)
            CHECK_VKCMD(tableVk.AllocateMemory(m_vkDevice, &memAlloc, nullptr, mem) );
...
#undef MK_ENUM
  ...
  ~CmdBuffer()
  {
    ...
    {
      if(buf != VK_NULL_HANDLE && tableVk.FreeCommandBuffers)
        tableVk.FreeCommandBuffers(m_vkDevice, pool, 1, &buf);

      if(pool != VK_NULL_HANDLE && tableVk.DestroyCommandPool)
        tableVk.DestroyCommandPool(m_vkDevice, pool, nullptr);

      if(execFence != VK_NULL_HANDLE && tableVk.DestroyFence)
        tableVk.DestroyFence(m_vkDevice, execFence, nullptr);
    }
...
#define CHECK_CBSTATE(s) \
  ...
  { \
      if(state != (s) ) \
      { \
          Log::Write(Log::Level::Error, std::string("Expecting state " #s " from ") + __FUNCTION__ + ", in " + StateString(state) ); \
          return false; \
      } \
  \
  }while(0)
  ...
  bool Init(const VulkanDebugObjectNamer& namer, VkDevice device, uint32_t queueFamilyIndex)
  {
    ...
    if(tableVk.CreateCommandPool)
      CHECK_VKCMD(tableVk.CreateCommandPool(m_vkDevice, &cmdPoolInfo, nullptr, &pool) );

    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_COMMAND_POOL, (uint64_t)pool, "helloxr command pool") );
    ...
    if(tableVk.AllocateCommandBuffers)
      CHECK_VKCMD(tableVk.AllocateCommandBuffers(m_vkDevice, &cmd, &buf) );

    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_COMMAND_BUFFER, (uint64_t)buf, "helloxr command buffer") );

    VkFenceCreateInfo fenceInfo {VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};

    if(tableVk.CreateFence)
      CHECK_VKCMD(tableVk.CreateFence(m_vkDevice, &fenceInfo, nullptr, &execFence) );

    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_FENCE, (uint64_t)execFence, "helloxr fence") );
  ...
  bool Begin()
  {
    ...
    if(tableVk.BeginCommandBuffer)
      CHECK_VKCMD(tableVk.BeginCommandBuffer(buf, &cmdBeginInfo) );
  ...
  bool End()
  {
    ...
    if(tableVk.EndCommandBuffer)
      CHECK_VKCMD(tableVk.EndCommandBuffer(buf) );
  ...
  bool Exec(VkQueue queue)
  {
    ...
    if(tableVk.QueueSubmit)
      CHECK_VKCMD(tableVk.QueueSubmit(queue, 1, &submitInfo, execFence) );
  ...
  bool Wait()
  {
    ...
    for(int i = 0; i < 5; ++i)
    {
      ...
      if(tableVk.WaitForFences)
        res = tableVk.WaitForFences(m_vkDevice, 1, &execFence, VK_TRUE, timeoutNs);
  ...
  bool Reset()
  {
    if(state != CmdBufferState::Initialized)
    {
      ...
      if(tableVk.ResetFences)
        CHECK_VKCMD(tableVk.ResetFences(m_vkDevice, 1, &execFence) );

      if(tableVk.ResetCommandBuffer)
        CHECK_VKCMD(tableVk.ResetCommandBuffer(buf, 0) );
    ...
    if(m_vkDevice != nullptr)
    {
      for(auto& si : shaderInfo)
      {
        if(si.module != VK_NULL_HANDLE && tableVk.DestroyShaderModule)
          tableVk.DestroyShaderModule(m_vkDevice, shaderInfo[0].module, nullptr);
...
private:
  ...
  void Load(uint32_t index, const std::vector<uint32_t>& code)
  {
    ...
    if(tableVk.CreateShaderModule)
      CHECK_VKCMD(tableVk.CreateShaderModule(m_vkDevice, &modInfo, nullptr, &si.module) );
...
struct VertexBufferBase
{
  ...
  ~VertexBufferBase()
  {
    if(m_vkDevice != nullptr)
    {
      if(idxBuf != VK_NULL_HANDLE && tableVk.DestroyBuffer)
        tableVk.DestroyBuffer(m_vkDevice, idxBuf, nullptr);

      if(idxMem != VK_NULL_HANDLE && tableVk.FreeMemory)
        tableVk.FreeMemory(m_vkDevice, idxMem, nullptr);

      if(vtxBuf != VK_NULL_HANDLE && tableVk.DestroyBuffer)
        tableVk.DestroyBuffer(m_vkDevice, vtxBuf, nullptr);

      if(vtxMem != VK_NULL_HANDLE && tableVk.FreeMemory)
        tableVk.FreeMemory(m_vkDevice, vtxMem, nullptr);
    }
...
protected:
  ...
  void AllocateBufferMemory(VkBuffer buf, VkDeviceMemory* mem) const
  {
    ...
    if(tableVk.GetBufferMemoryRequirements)
      tableVk.GetBufferMemoryRequirements(m_vkDevice, buf, &memReq);
...
template <typename T> struct VertexBuffer : public VertexBufferBase
{
  bool Create(uint32_t idxCount, uint32_t vtxCount)
  {
    ...
    if(tableVk.CreateBuffer)
      CHECK_VKCMD(tableVk.CreateBuffer(m_vkDevice, &bufInfo, nullptr, &idxBuf) );

    AllocateBufferMemory(idxBuf, &idxMem);

    if(tableVk.BindBufferMemory)
      CHECK_VKCMD(tableVk.BindBufferMemory(m_vkDevice, idxBuf, idxMem, 0) );

    bufInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    bufInfo.size = sizeof(T) * vtxCount;

    if(tableVk.CreateBuffer)
      CHECK_VKCMD(tableVk.CreateBuffer(m_vkDevice, &bufInfo, nullptr, &vtxBuf) );

    AllocateBufferMemory(vtxBuf, &vtxMem);

    if(tableVk.BindBufferMemory)
      CHECK_VKCMD(tableVk.BindBufferMemory(m_vkDevice, vtxBuf, vtxMem, 0) );
  ...
  void UpdateIndices(const uint16_t* data, uint32_t elements, uint32_t offset = 0)
  {
    ...
    if(tableVk.MapMemory)
      CHECK_VKCMD(tableVk.MapMemory(m_vkDevice, idxMem, sizeof(map[0] ) * offset, sizeof(map[0] ) * elements, 0, (void**)&map) );

    for(size_t i = 0; i < elements; ++i)
      map[i] = data[i];

    if(tableVk.UnmapMemory)
      tableVk.UnmapMemory(m_vkDevice, idxMem);
  }
  ...
  void UpdateVertices(const T* data, uint32_t elements, uint32_t offset = 0)
  {
    ...
    if(tableVk.MapMemory)
      CHECK_VKCMD(tableVk.MapMemory(m_vkDevice, vtxMem, sizeof(map[0] ) * offset, sizeof(map[0] ) * elements, 0, (void**)&map) );

    for(size_t i = 0; i < elements; ++i)
      map[i] = data[i];

    if(tableVk.UnmapMemory)
      tableVk.UnmapMemory(m_vkDevice, vtxMem);
  }
};
...
struct RenderPass
{
  ...
  bool Create(const VulkanDebugObjectNamer& namer, VkDevice device, VkFormat aColorFmt, VkFormat aDepthFmt)
  {
    ...
    if(tableVk.CreateRenderPass)
      CHECK_VKCMD(tableVk.CreateRenderPass(m_vkDevice, &rpInfo, nullptr, &pass) );

    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_RENDER_PASS, (uint64_t)pass, "helloxr render pass") );
  ...
  ~RenderPass()
  {
      if(m_vkDevice != nullptr)
      {
        if(pass != VK_NULL_HANDLE && tableVk.DestroyRenderPass)
          tableVk.DestroyRenderPass(m_vkDevice, pass, nullptr);
      }
...
struct RenderTarget
{
  ...
  ~RenderTarget()
  {
    if(m_vkDevice != nullptr)
    {
      if(fb != VK_NULL_HANDLE && tableVk.DestroyFramebuffer)
        tableVk.DestroyFramebuffer(m_vkDevice, fb, nullptr);

      if(colorView != VK_NULL_HANDLE && tableVk.DestroyImageView)
        tableVk.DestroyImageView(m_vkDevice, colorView, nullptr);

      if(depthView != VK_NULL_HANDLE && tableVk.DestroyImageView)
        tableVk.DestroyImageView(m_vkDevice, depthView, nullptr);
    }
  ...
  void Create(const VulkanDebugObjectNamer& namer, VkDevice device, VkImage aColorImage, VkImage aDepthImage, VkExtent2D size, RenderPass& renderPass)
  {
    m_vkDevice = device;
    ...
    if(colorImage != VK_NULL_HANDLE)
    {
      ...
      if(tableVk.CreateImageView)
        CHECK_VKCMD(tableVk.CreateImageView(m_vkDevice, &colorViewInfo, nullptr, &colorView) );

      CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t)colorView, "helloxr color image view") );
    ...
    if(depthImage != VK_NULL_HANDLE)
    {
      ...
      if(tableVk.CreateImageView)
        CHECK_VKCMD(tableVk.CreateImageView(m_vkDevice, &depthViewInfo, nullptr, &depthView) );

      CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t)depthView, "helloxr depth image view") );
    ...
    if(tableVk.CreateFramebuffer)
      CHECK_VKCMD(tableVk.CreateFramebuffer(m_vkDevice, &fbInfo, nullptr, &fb) );

    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_FRAMEBUFFER, (uint64_t)fb, "helloxr framebuffer") );
  }
...
struct PipelineLayout
{
  ...
  ~PipelineLayout()
  {
    if(m_vkDevice != nullptr)
    {
      if(layout != VK_NULL_HANDLE && tableVk.DestroyPipelineLayout)
        tableVk.DestroyPipelineLayout(m_vkDevice, layout, nullptr);
    }
  ...
  void Create(VkDevice device)
  {
    ...
    if(tableVk.CreatePipelineLayout)
      CHECK_VKCMD(tableVk.CreatePipelineLayout(m_vkDevice, &pipelineLayoutCreateInfo, nullptr, &layout) );
  }
...
struct Pipeline
{
  ...
  void Create(VkDevice device, VkExtent2D size, const PipelineLayout& layout, const RenderPass& rp, const ShaderProgram& sp, const VertexBufferBase& vb)
  {
    m_vkDevice = device;
    ...
    if(tableVk.CreateGraphicsPipelines)
      CHECK_VKCMD(tableVk.CreateGraphicsPipelines(m_vkDevice, VK_NULL_HANDLE, 1, &pipeInfo, nullptr, &pipe) );
  }
  ...
  void Release()
  {
    if(m_vkDevice != nullptr)
    {
      if(pipe != VK_NULL_HANDLE && tableVk.DestroyPipeline)
        tableVk.DestroyPipeline(m_vkDevice, pipe, nullptr);
    }
...
struct DepthBuffer
{
  ...
  ~DepthBuffer()
  {
    if(m_vkDevice != nullptr)
    {
      if(depthImage != VK_NULL_HANDLE && tableVk.DestroyImage)
        tableVk.DestroyImage(m_vkDevice, depthImage, nullptr);

      if(depthMemory != VK_NULL_HANDLE && tableVk.FreeMemory)
        tableVk.FreeMemory(m_vkDevice, depthMemory, nullptr);
    }
  ...
  void Create(const VulkanDebugObjectNamer& namer, VkDevice device, MemoryAllocator* memAllocator, VkFormat depthFormat, const XrSwapchainCreateInfo& swapchainCreateInfo)
  {
    m_vkDevice = device;
    ...
    if(tableVk.CreateImage)
      CHECK_VKCMD(tableVk.CreateImage(device, &imageInfo, nullptr, &depthImage) );

    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_IMAGE, (uint64_t)depthImage, "helloxr fallback depth image") );

    VkMemoryRequirements memRequirements {};

    if(tableVk.GetImageMemoryRequirements)
      tableVk.GetImageMemoryRequirements(device, depthImage, &memRequirements);

    memAllocator->Allocate(memRequirements, &depthMemory, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    CHECK_VKCMD(namer.SetName(VK_OBJECT_TYPE_DEVICE_MEMORY, (uint64_t)depthMemory, "helloxr fallback depth image memory") );

    if(tableVk.BindImageMemory)
      CHECK_VKCMD(tableVk.BindImageMemory(device, depthImage, depthMemory, 0) );
  }
  ...
  void TransitionLayout(CmdBuffer* cmdBuffer, VkImageLayout newLayout)
  {
    ...
    if(tableVk.CmdPipelineBarrier)
      tableVk.CmdPipelineBarrier(cmdBuffer->buf, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, 0, 0, nullptr, 0, nullptr, 1, &depthBarrier);
...
struct SwapchainImageContext
{
 ...
 std::vector<XrSwapchainImageBaseHeader*> Create(const VulkanDebugObjectNamer& namer, VkDevice device, MemoryAllocator* memAllocator, uint32_t capacity, const XrSwapchainCreateInfo& swapchainCreateInfo, const PipelineLayout& layout, const ShaderProgram& sp, const VertexBuffer<Geometry::Vertex>& vb)
 {
    m_vkDevice = device;
    m_namer = namer;
...
struct Swapchain
{
  ...
  void Release()
  {
    if(m_vkDevice)
    {
      ...
      if(swapchain && tableVk.DestroySwapchainKHR)
        tableVk.DestroySwapchainKHR(m_vkDevice, swapchain, nullptr);

      if(readyFence && tableVk.DestroyFence)
        tableVk.DestroyFence(m_vkDevice, readyFence, nullptr);
    }

    if(m_vkInstance && surface && tableVk.DestroySurfaceKHR)
      tableVk.DestroySurfaceKHR(m_vkInstance, surface, nullptr);
...
#error CreateSurface not supported on this OS
  ...
  if(tableVk.GetPhysicalDeviceSurfaceCapabilitiesKHR)
    CHECK_VKCMD(tableVk.GetPhysicalDeviceSurfaceCapabilitiesKHR(m_vkPhysicalDevice, surface, &surfCaps) );
  ...
  if(tableVk.GetPhysicalDeviceSurfaceFormatsKHR)
    CHECK_VKCMD(tableVk.GetPhysicalDeviceSurfaceFormatsKHR(m_vkPhysicalDevice, surface, &surfFmtCount, nullptr) );

  std::vector<VkSurfaceFormatKHR> surfFmts(surfFmtCount);

  if(tableVk.GetPhysicalDeviceSurfaceFormatsKHR)
    CHECK_VKCMD(tableVk.GetPhysicalDeviceSurfaceFormatsKHR(m_vkPhysicalDevice, surface, &surfFmtCount, &surfFmts[0] ) );
  ...
  if(tableVk.GetPhysicalDeviceSurfacePresentModesKHR)
    CHECK_VKCMD(tableVk.GetPhysicalDeviceSurfacePresentModesKHR(m_vkPhysicalDevice, surface, &presentModeCount, nullptr) );

  std::vector<VkPresentModeKHR> presentModes(presentModeCount);

  if(tableVk.GetPhysicalDeviceSurfacePresentModesKHR)
    CHECK_VKCMD(tableVk.GetPhysicalDeviceSurfacePresentModesKHR(m_vkPhysicalDevice, surface, &presentModeCount, &presentModes[0] ) );
  ...
  {
    CHECK_VKCMD(tableVk.GetPhysicalDeviceSurfaceSupportKHR(m_vkPhysicalDevice, m_queueFamilyIndex, surface, &presentable) );
    CHECK(presentable);
  }
  ...
  if(tableVk.CreateSwapchainKHR)
    CHECK_VKCMD(tableVk.CreateSwapchainKHR(m_vkDevice, &swapchainInfo, nullptr, &swapchain) );

  // Fence to throttle host on Acquire
  VkFenceCreateInfo fenceInfo {VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};

  if(tableVk.CreateFence)
    CHECK_VKCMD(tableVk.CreateFence(m_vkDevice, &fenceInfo, nullptr, &readyFence) );

  swapchainCount = 0;

  if(tableVk.GetSwapchainImagesKHR)
    CHECK_VKCMD(tableVk.GetSwapchainImagesKHR(m_vkDevice, swapchain, &swapchainCount, nullptr) );

  assert(swapchainCount < maxImages);

  if(tableVk.GetSwapchainImagesKHR)
    CHECK_VKCMD(tableVk.GetSwapchainImagesKHR(m_vkDevice, swapchain, &swapchainCount, image) );
...
void Swapchain::Prepare(VkCommandBuffer buf)
{
  ...
  for(uint32_t i = 0; i < swapchainCount; ++i)
  {
    ...
    if(tableVk.CmdPipelineBarrier)
      tableVk.CmdPipelineBarrier(buf, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &imgBarrier);
  }
}
...
void Swapchain::Wait()
{
  if(presentFence)
  {
    // Wait for the fence...
    if(tableVk.WaitForFences)
      CHECK_VKCMD(tableVk.WaitForFences(m_vkDevice, 1, &presentFence, VK_TRUE, UINT64_MAX) );

    // ...then reset the fence for future Acquire calls
    if(tableVk.ResetFences)
      CHECK_VKCMD(tableVk.ResetFences(m_vkDevice, 1, &presentFence) );
...
void Swapchain::Acquire(VkSemaphore readySemaphore)
{
  ...
  if(tableVk.AcquireNextImageKHR)
    CHECK_VKCMD(tableVk.AcquireNextImageKHR(m_vkDevice, swapchain, UINT64_MAX, readySemaphore, presentFence, &renderImageIdx) );
}
...
void Swapchain::Present(VkQueue queue, VkSemaphore drawComplete)
{
  ...
  if(tableVk.QueuePresentKHR)
    res = tableVk.QueuePresentKHR(queue, &presentInfo);
  ...
  CHECK_VKRESULT(res, "tableVk.QueuePresentKHR");
}
...
#include "frag.spv"
    ...
    m_drawBuffer.Init(m_vkDevice, &m_memAllocator, { {0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Geometry::Vertex, Position) }, {1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Geometry::Vertex, Color) } } );
  ...
  std::vector<XrSwapchainImageBaseHeader*> AllocateSwapchainImageStructs(uint32_t capacity, const XrSwapchainCreateInfo& swapchainCreateInfo) override
  {
    ...
    std::vector<XrSwapchainImageBaseHeader*> bases = swapchainImageContext.Create(m_namer, m_vkDevice, &m_memAllocator, capacity, swapchainCreateInfo, m_pipelineLayout, m_shaderProgram, m_drawBuffer);
  ...
  void RenderView(const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage, int64_t /*swapchainFormat*/, const std::vector<Cube>& cubes) override
  {
    ...
    if(tableVk.CmdBeginRenderPass)
      tableVk.CmdBeginRenderPass(m_cmdBuffer.buf, &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);

    if(tableVk.CmdBindPipeline)
      tableVk.CmdBindPipeline(m_cmdBuffer.buf, VK_PIPELINE_BIND_POINT_GRAPHICS, swapchainContext->pipe.pipe);

    // Bind index and vertex buffers
    if(tableVk.CmdBindIndexBuffer)
      tableVk.CmdBindIndexBuffer(m_cmdBuffer.buf, m_drawBuffer.idxBuf, 0, VK_INDEX_TYPE_UINT16);

    VkDeviceSize offset = 0;

    if(tableVk.CmdBindVertexBuffers)
      tableVk.CmdBindVertexBuffers(m_cmdBuffer.buf, 0, 1, &m_drawBuffer.vtxBuf, &offset);
    ...
    for(const Cube& cube : cubes)
    {
      ...
      if(tableVk.CmdPushConstants)
        tableVk.CmdPushConstants(m_cmdBuffer.buf, m_pipelineLayout.layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(mvp.m), &mvp.m[0] );

      // draw the cube
      if(tableVk.CmdDrawIndexed)
        tableVk.CmdDrawIndexed(m_cmdBuffer.buf, m_drawBuffer.count.idx, 1, 0, 0, 0);
    }

    if(tableVk.CmdEndRenderPass)
      tableVk.CmdEndRenderPass(m_cmdBuffer.buf);
 ...
 protected:
  ...
  static std::string BlahVkObjectTypeToString(VkObjectType objectType)
  {
    std::string objName;
...
#define MK_OBJECT_TYPE_CASE(name) \
  ...
  VkBool32 debugMessage(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageTypes, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData)
  {
    ...
    if(pCallbackData->objectCount > 0)
    {
      ...
      objName = BlahVkObjectTypeToString(objectType);
...
struct VulkanGraphicsPluginLegacy : public VulkanGraphicsPlugin
{
  ...
  virtual XrResult CreateVulkanInstanceKHR(XrInstance instance, const XrVulkanInstanceCreateInfoKHR* createInfo, VkInstance* vulkanInstance, VkResult* vulkanResult) override
  {
    ...
    XrResult xresult = XR_ERROR_VALIDATION_FAILURE;
    VkResult vresult = VK_ERROR_OUT_OF_HOST_MEMORY;

    InitOpenXr();

    if(tableXr.GetInstanceProcAddr)
    {
      xresult = tableXr.GetInstanceProcAddr(instance, "xrGetVulkanInstanceExtensionsKHR", reinterpret_cast<PFN_xrVoidFunction*>( &pfnGetVulkanInstanceExtensionsKHR) );
      CHECK_XRCMD(xresult);
    }
    ...
    {
      xresult = pfnGetVulkanInstanceExtensionsKHR(instance, createInfo->systemId, 0, &extensionNamesSize, nullptr);
      CHECK_XRCMD(xresult);
    }
    ...
    if(pfnGetVulkanInstanceExtensionsKHR)
    {
      xresult = pfnGetVulkanInstanceExtensionsKHR(instance, createInfo->systemId, extensionNamesSize, &extensionNamesSize, &extensionNames[0] );
      CHECK_XRCMD(xresult);
    }

    {
      ...
      LogVulkanExtensions("Vulkan Instance Extensions, requested by application", extensions, extensions.size() - createInfo->vulkanCreateInfo->enabledExtensionCount);

      VkInstanceCreateInfo instInfo;
      memcpy( &instInfo, createInfo->vulkanCreateInfo, sizeof(VkInstanceCreateInfo) );

      instInfo.enabledExtensionCount = (uint32_t)extensions.size();
      instInfo.ppEnabledExtensionNames = extensions.empty() ? nullptr : extensions.data();

      PFN_vkCreateInstance pfnCreateInstance = 0;

      if(createInfo->pfnGetInstanceProcAddr)
        pfnCreateInstance = (PFN_vkCreateInstance)createInfo->pfnGetInstanceProcAddr(nullptr, "vkCreateInstance");

      if(pfnCreateInstance)
        vresult = pfnCreateInstance(&instInfo, createInfo->vulkanAllocator, vulkanInstance);

      *vulkanResult = vresult;
    }
  ...
  virtual XrResult CreateVulkanDeviceKHR(XrInstance instance, const XrVulkanDeviceCreateInfoKHR* createInfo, VkDevice* vulkanDevice, VkResult* vulkanResult) override
  {
    ...
    if(tableXr.GetInstanceProcAddr)
    {
      xresult = tableXr.GetInstanceProcAddr(instance, "xrGetVulkanDeviceExtensionsKHR", reinterpret_cast<PFN_xrVoidFunction*>( &pfnGetVulkanDeviceExtensionsKHR) );
      CHECK_XRCMD(xresult);
    }

    uint32_t deviceExtensionNamesSize = 0;

    if(pfnGetVulkanDeviceExtensionsKHR)
    {
      xresult = pfnGetVulkanDeviceExtensionsKHR(instance, createInfo->systemId, 0, &deviceExtensionNamesSize, nullptr);
      CHECK_XRCMD(xresult);
    }

    std::vector<char> deviceExtensionNames(deviceExtensionNamesSize);

    if(deviceExtensionNamesSize > 0 && pfnGetVulkanDeviceExtensionsKHR)
    {
      xresult = pfnGetVulkanDeviceExtensionsKHR(instance, createInfo->systemId, deviceExtensionNamesSize, &deviceExtensionNamesSize, &deviceExtensionNames[0] );
      CHECK_XRCMD(xresult);
    }

    {
      ...
      LogVulkanExtensions("Vulkan Device Extensions, requested by application", extensions, extensions.size() - createInfo->vulkanCreateInfo->enabledExtensionCount);

      VkPhysicalDeviceFeatures features;
      memcpy( &features, createInfo->vulkanCreateInfo->pEnabledFeatures, sizeof(VkPhysicalDeviceFeatures) );

      VkDeviceCreateInfo deviceInfo;
      memcpy( &deviceInfo, createInfo->vulkanCreateInfo, sizeof(VkDeviceCreateInfo) );

      deviceInfo.pEnabledFeatures = &features;
      deviceInfo.enabledExtensionCount = (uint32_t)extensions.size();
      deviceInfo.ppEnabledExtensionNames = extensions.empty() ? nullptr : extensions.data();

      PFN_vkCreateDevice pfnCreateDevice = 0;

      if(createInfo->pfnGetInstanceProcAddr)
        pfnCreateDevice = (PFN_vkCreateDevice)createInfo->pfnGetInstanceProcAddr(m_vkInstance, "vkCreateDevice");

      if(pfnCreateDevice)
        vresult = pfnCreateDevice(m_vkPhysicalDevice, &deviceInfo, createInfo->vulkanAllocator, vulkanDevice);
  ...
  virtual XrResult GetVulkanGraphicsDevice2KHR(XrInstance instance, const XrVulkanGraphicsDeviceGetInfoKHR* getInfo, VkPhysicalDevice* vulkanPhysicalDevice) override
  {
    ...
    if(tableXr.GetInstanceProcAddr)
      CHECK_XRCMD(tableXr.GetInstanceProcAddr(instance, "xrGetVulkanGraphicsDeviceKHR", reinterpret_cast<PFN_xrVoidFunction*>( &pfnGetVulkanGraphicsDeviceKHR) ) );
    ...
    {
      result = pfnGetVulkanGraphicsDeviceKHR(instance, getInfo->systemId, getInfo->vulkanInstance, vulkanPhysicalDevice);
      CHECK_XRCMD(result);
    }
  ...
  virtual XrResult GetVulkanGraphicsRequirements2KHR(XrInstance instance, XrSystemId systemId, XrGraphicsRequirementsVulkan2KHR* graphicsRequirements) override
  {
    ...
    if(tableXr.GetInstanceProcAddr)
    {
      result = tableXr.GetInstanceProcAddr(instance, "xrGetVulkanGraphicsRequirementsKHR", reinterpret_cast<PFN_xrVoidFunction*>( &pfnGetVulkanGraphicsRequirementsKHR) );
      CHECK_XRCMD(result);
    }
    ...
    {
      result = pfnGetVulkanGraphicsRequirementsKHR(instance, systemId, &legacyRequirements);
      CHECK_XRCMD(result);
    }
