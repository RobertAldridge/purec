  ...
  OculusAchievementInterface->QueryAchievementDescriptions(
      *UserId.Get(),
      FOnQueryAchievementsCompleteDelegate::CreateUObject(
          this, &UOSSAchievementWidget::OnQueryAchievementDescriptionsComplete));
}

void UOSSAchievementWidget::OnQueryAchievementDescriptionsComplete(
  ...
  if (bWasSuccessful) {
    ...
    OculusAchievementInterface->QueryAchievements(
        PlayerId,
        FOnQueryAchievementsCompleteDelegate::CreateUObject(
            this, &UOSSAchievementWidget::OnQueryAchievementsComplete));
  } else {
...
void UOSSAchievementWidget::OnQueryAchievementsComplete(
    const FUniqueNetId& PlayerId,
    const bool bWasSuccessful) {
...
void UOSSAchievementWidget::OnAchievementsWritten(
    const FUniqueNetId& PlayerId,
    const bool bWasSuccessful) {
...
void UOSSAchievementWidget::ShowAchievementInfo(const FString& AchievementName) {
  ...
  UE_LOG_ONLINE(
      Display, TEXT("Got  Achievement: %s, Progress: %f"), *Achievement.Id, Achievement.Progress);
}
...
void UOSSAchievementWidget::UnlockAchievement(const FString& AchievementName) {
  ...
  if (!Achievement.Id.IsEmpty()) {
    ...
    (Achievement.Progress > 0)
        ? WriteObject->SetIntStat(*AchievementName, 0)
        : WriteObject->SetIntStat(*AchievementName, 1); // toggle the acheievement
    FOnlineAchievementsWriteRef WriteObjectRef = WriteObject.ToSharedRef();
    OculusAchievementInterface->WriteAchievements(
        *UserId.Get(),
        WriteObjectRef,
        FOnAchievementsWrittenDelegate::CreateUObject(
            this, &UOSSAchievementWidget::OnAchievementsWritten));
  }
}
