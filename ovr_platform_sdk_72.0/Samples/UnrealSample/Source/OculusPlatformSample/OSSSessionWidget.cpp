...
void UOSSSessionWidget::ReadOnlineFriends() {
  ...
  OculusFriendsInterface->ReadFriendsList(
      ...
      FOnReadFriendsListComplete::CreateUObject(
          this, &UOSSSessionWidget::OnReadFriendsListComplete));
}

void UOSSSessionWidget::OnReadFriendsListComplete(
    int32 LocalUserNum,
  ...
  if (bWasSuccessful) {
    ...
    UE_LOG_ONLINE(
        Display, TEXT("Online Friends list loaded.  Count of friends: %d"), Friends.Num());
    ...
    if (!OnFindFriendSessionCompleteDelegate.IsBound()) {
      OnFindFriendSessionCompleteDelegate = FOnFindFriendSessionCompleteDelegate::CreateUObject(
          this, &UOSSSessionWidget::OnFindFriendSessionComplete);
...
void UOSSSessionWidget::OnFindFriendSessionComplete(
...
void UOSSSessionWidget::InviteFriendToSession(const FString& FriendName) {
  ...
  if (FriendsToInvite.Contains(FriendName)) {
    auto OculusSessionInterface = Online::GetSessionInterface();
    bool bSuccess = OculusSessionInterface->SendSessionInviteToFriend(
        0, TEXT("Game"), FriendsToInvite[FriendName].Get());
    if (!bSuccess)
...
void UOSSSessionWidget::OnJoinSessionComplete(
    FName SessionName,
  ...
  if (Session) {
    ...
    LastPlayerCount =
        Session->RegisteredPlayers
            .Num(); // save off the number of players in the session when I joined to compare later

    if (*Session->OwningUserId ==
        *Online::GetIdentityInterface()->GetUniquePlayerId(0)) // I am the owner
    {
...
void UOSSSessionWidget::CreateSession() {
  ...
  if (!OnCreateSessionCompleteDelegate.IsBound()) {
    OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(
        this, &UOSSSessionWidget::OnCreateSessionComplete);
...
void UOSSSessionWidget::StartSession(FName SessionName) {
  ...
  if (!OnStartSessionCompleteDelegate.IsBound()) {
    OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(
        this, &UOSSSessionWidget::OnStartSessionComplete);
...
void UOSSSessionWidget::EndSession(FName SessionName) {
  ...
  if (!OnEndSessionCompleteDelegate.IsBound()) {
    OnEndSessionCompleteDelegate = FOnEndSessionCompleteDelegate::CreateUObject(
        this, &UOSSSessionWidget::OnEndSessionComplete);
...
void UOSSSessionWidget::DestroySession(FName SessionName) {
  ...
  if (!OnDestroySessionCompleteDelegate.IsBound()) {
    OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(
        this, &UOSSSessionWidget::OnDestroySessionComplete);
...
void UOSSSessionWidget::OnSessionUserInviteAccepted(
    const bool bWasSuccessful,
...
bool UOSSSessionWidget::StartMatchmaking(const FString& PoolName) {
  ...
  if (!OnMatchmakingCompleteDelegate.IsBound()) {
    OnMatchmakingCompleteDelegate = FOnMatchmakingCompleteDelegate::CreateUObject(
        this, &UOSSSessionWidget::OnMatchmakingComplete);
  ...
  SearchSettings->QuerySettings.Set(
      FName(TEXT("OCULUSPOOL")), PoolName, EOnlineComparisonOp::Equals);

  TSharedRef<FOnlineSessionSearch> SearchSettingsRef = SearchSettings.ToSharedRef();

  // Do the search
  return OculusSessionInterface->StartMatchmaking(
      LocalPlayers, TEXT("Game"), *SessionSettings, SearchSettingsRef);
}
...
bool UOSSSessionWidget::CancelMatchmaking(FName SessionName) {
  ...
  if (!OnCancelMatchmakingCompleteDelegate.IsBound()) {
    OnCancelMatchmakingCompleteDelegate = FOnCancelMatchmakingCompleteDelegate::CreateUObject(
        this, &UOSSSessionWidget::OnCancelMatchmakingComplete);
...
void UOSSSessionWidget::OnCancelMatchmakingComplete(FName SessionName, bool bWasSuccessful) {
  UE_LOG_ONLINE(
      Display,
      TEXT("Matchmaking Cancel returned: %s"),
...
int32 UOSSSessionWidget::RefreshPlayerCountInSession() {
  ...
  if (Session->RegisteredPlayers.Num() != LastPlayerCount) {
    Session->RegisteredPlayers.Num() > LastPlayerCount ? OnSessionPlayerCountChanged(true)
                                                       : OnSessionPlayerCountChanged(false);
...
bool UOSSSessionWidget::InAValidSession() {
  ...
  UE_LOG_ONLINE(
      Display,
      TEXT("InAValidSession returned: %s"),
      ((Session->SessionState != EOnlineSessionState::NoSession) ? TEXT("true") : TEXT("false")));
