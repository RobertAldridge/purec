...
void AOculusNetDriverActor::OnLoginComplete(
    int32 LocalUserNum,
    bool bWasSuccessful,
...
void AOculusNetDriverActor::Tick(float DeltaTime) {
  ...
  if (NetMode == NM_ListenServer) {
    ...
    NetDriverStatus = FString::Printf(
        TEXT("Pending: %d Connected Clients: %d"), PendingClients, ConnectedClients);
  } else if (NetMode == NM_Client) {
    NetModeStatus = FString::Printf(TEXT("Client: %s"), *PlayerID);
    if (NetDriver && NetDriver->IsAvailable()) {
      NetDriverStatus = FString::Printf(
          TEXT("NetDriver is connected to %s"),
          *NetDriver->ServerConnection->RemoteAddressToString());
    } else {
