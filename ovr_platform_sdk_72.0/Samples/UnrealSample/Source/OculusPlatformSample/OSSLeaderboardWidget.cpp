...
void UOSSLeaderboardWidget::ReadLeaderboardData(const FName& LBName) {
  ...
  UE_LOG_ONLINE(
      Display, TEXT("Trying to get leaderboard: %s data from server"), *LBName.ToString());
  ...
  if (!OnLeaderboardReadCompleteDelegate.IsBound()) {
    OnLeaderboardReadCompleteDelegate = FOnLeaderboardReadCompleteDelegate::CreateUObject(
        this, &UOSSLeaderboardWidget::OnLeaderboardReadComplete);
    OculusLeaderboardInterface->AddOnLeaderboardReadCompleteDelegate_Handle(
        OnLeaderboardReadCompleteDelegate);
  }
...
void UOSSLeaderboardWidget::OnLeaderboardReadComplete(bool bInIsError) {
  OnLeaderboardReadCompleteBP(
      bInIsError, LeaderboardRead->LeaderboardName); // notify the BP that the leaderboard read is
                                                     // complete to clear the UI for new data
  ...
  for (auto row : LeaderboardRead->Rows) {
    UE_LOG_ONLINE(
        Display,
        TEXT("Found Leaderboard entry: PlayerID: %s, Nickname: %s, Rank: %i, "),
    ...
    for (auto column : ColumnArray) {
      ...
      AddLeaderboardRowToUI(
          *row.PlayerId->ToString(),
          *row.NickName,
          FString::FromInt(row.Rank),
...
void UOSSLeaderboardWidget::WriteLeaderboardData(const FName& LBName, int32 ValueToWrite) {
  ...
  UE_LOG_ONLINE(
      Display,
      TEXT("Trying to write to leaderboard: %s with data: %i"),
      *LBName.ToString(),
      ValueToWrite);
